...
export default function ExpandedPlayer() {
  ...
  const spotifyUrl = currentItem?.spotifyTrackId
    ...
    : null;
  const shouldShow = state.isExpanded && state.isVisible && !!currentSong;
  ...
  // AnimatePresence is rendered INSIDE the portal so exit animations
  // happen within #modal-root, avoiding removeChild conflicts.
  return createPortal(
    <AnimatePresence>
      {shouldShow && (
        <motion.div
            ...
            <div className="text-center mb-6">
              ...
              {isSpotify && (
                <span className="inline-block mt-2 text-xs text-emerald-400 bg-emerald-400/10 px-2 py-0.5 rounded-full">
                  {isPreview ? "Preview — solo 30 segundos" : "Reproduciendo con Spotify"}
                </span>
              )}
            </div>
              ...
              <div className="mb-6 max-w-md mx-auto space-y-3">
                ...
                {isPreview && spotifyUrl && (
                  <a
                    ...
                    className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl bg-emerald-500/15 text-emerald-400 text-sm font-medium hover:bg-emerald-500/25 transition-colors active:scale-[0.98]"
                  >
